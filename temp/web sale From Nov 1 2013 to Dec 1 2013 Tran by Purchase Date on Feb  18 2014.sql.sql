...
--drop table report_db..WebSaleCC2013Nov

drop table report_db..WebSaleCCTran2013Nov_feb18
drop table report_db..WebSaleTran2013Nov_feb18
...
into report_db..WebSaleCCTran2013Nov_feb18
...
into report_db..WebSaleTran2013Nov_feb18
...
from report_db..WebSaleCCTran2013Nov_feb14 a, report_db..louie b
where a.xactionId = b.xactionId
order by b.id

select * from CreditCardTransaction cct where dateCreated >= "Nov 1 2013" and cct.CCTranStatusId = 4
order by cct.dateCreated

select * from report_db..WebSaleCCTran2013Nov
--summary
select * from report_db..WebSaleCC2013Nov

drop table report_db..WebSaleCC2013Nov_feb13
select sum(p.cost) as 'cost', sum(p.tax) as 'tax', sum(p.costUSD) as 'costUSD', sum(p.taxUSD) as 'taxUSD',
       pt.purchaseTypeId, pt.description as 'paymentType', ct.countryName as 'taxCountry', jt.jurisdictionName as 'taxJurisdiction', cc.cardTypeId,
       b.merchantId 
into report_db..WebSaleCC2013Nov_feb13
from Purchase p, PurchaseType pt, CountryTaxRate ct, JurisdictionTaxRate jt, CreditCard cc, BillingLocation b, CreditCardTransaction cct
where p.purchaseTypeId = pt.purchaseTypeId
and p.creditCardId *= cc.creditCardId 
and p.taxCountryId = ct.countryId
...
select sum(p.cost) as 'cost', sum(p.tax) as 'tax', sum(p.costUSD) as 'costUSD', sum(p.taxUSD) as 'taxUSD',
       pt.purchaseTypeId, pt.description as 'paymentType', ct.countryName as 'taxCountry', jt.jurisdictionName as 'taxJurisdiction', cc.cardTypeId,
       b.merchantId 
into report_db..WebSale2013Nov_feb13
from Purchase p, PurchaseType pt, CountryTaxRate ct, JurisdictionTaxRate jt, CreditCard cc, BillingLocation b
where p.purchaseTypeId = pt.purchaseTypeId
and p.creditCardId *= cc.creditCardId 
and p.taxCountryId = ct.countryId
...
select * from report_db..WebSale2013Nov
select * from report_db..WebSale2013Nov_feb13
...
SELECT
    ...
    cnt.contentDesc,
    t.partialCardNum
  FROM report_db..WebSaleCCTran2013Nov_feb18 t, Accounting..CardType c, Accounting..Content cnt
 WHERE t.cardTypeId = c.cardTypeId
...
SELECT
    ...
    dateCreated,
    t.userId 
  FROM report_db..WebSaleTran2013Nov_feb18 t
