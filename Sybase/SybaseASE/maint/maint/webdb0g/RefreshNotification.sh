...
MAIL_MESSAGE_FILE="${SYBMAINT}/Refresh/output/${0}.mail.${SERVER_NAME}"
...
#---------------------------------------------#
sqsh -U${SQL_USER} -S${OPS_SERVER_NAME} -P${PASSWORD} -DDbaTasks <<EOQ1 >> ${LOG_FILE}
...
SELECT  @WeekDayCount = round(datepart(dd,getdate())/7, 0) + 1 
...
sqsh -U${SQL_USER} -S${OPS_SERVER_NAME} -P${PASSWORD} -DDbaTasks <<EOQ2 >> ${LOG_FILE}
...
sqsh -U${SQL_USER} -S${OPS_SERVER_NAME} -P${PASSWORD} -DDbaTasks <<EOQ3 >> ${LOG_FILE}
   ...
   \echo "We are going to refresh the test db server at 5:00PM, "${YEAR_MON_DAY} >> ${MAIL_MESSAGE_FILE}                      
   \echo " "  >> ${MAIL_MESSAGE_FILE}
